...
public_eps = read.csv(paste(mwd,"/", resdir, "/All.public_epitope_annotations.Zscores.csv", sep = ""), header = TRUE, sep = ",")
...
# Keep only this virus 
viral_ids = species_id$id[which(species_id$Species==virus)]
...
viral_plotfile =paste(mwd,"/",resdir, "/Manhattan_plots_",virus, "_", paste(groups_to_compare, collapse = "_"), ".pdf", sep = "")
...
for(indiv in statids){
 plot_df$Zscore = 0
  ...
  myplot = grid.arrange(grobs = lapply(pcount:pmax, function(i) plots[[i]]), ncol=1, nrow=n_plots_per_page, top = virus)
  print(myplot)
