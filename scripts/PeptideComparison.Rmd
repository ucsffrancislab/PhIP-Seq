...
ggplot(df, aes(x = exp(beta), y = -log10(pval),size=sig,fill=sig)) +
...
#	coord_cartesian(ylim = c(0, 5), xlim = c(-5, 5)) + # since some genes can have minuslog10padj of inf, we set these limits
...
```{r plot 11b}
ggplot(df, aes(x = exp(beta), y = -log10(pval))) +
	geom_vline(xintercept=0, linewidth = 0.5,col="gray60",linetype=2)+
	geom_point(aes(fill=sig,size=sig,shape=shape)) + scale_shape_identity() +
	geom_hline(yintercept=1.305, linewidth = 0.5, linetype=2)+
	scale_fill_manual(values=c("#AAAAAA","#2ED0FE"),label=c(">0.05","<0.05"),name=c("p-value")) +
...
```{r plot 13}
ggplot(df, aes(y = -log10(pval), x = exp(beta), col = group)) +
...
```{r plot 15}
ggplot(df, aes(y = -log10(pval), x = exp(beta))) +
	ggtitle("All Herpesviruses") +
	geom_vline(xintercept=0, linewidth = 0.5,col="gray60",linetype=2)+
	geom_hline(yintercept=1.305, linewidth = 0.5, linetype=2)+
#	coord_cartesian(ylim = c(0, 4), xlim = c(-5, 5)) +
	labs(x="exp(beta)", y=expression(paste("-log"[10]*"(",italic("P"),")"))) +
	geom_point(data = filter(df, group == "Other"), alpha = .1, size=2) +
...
```{r plot 17}
ggplot(df, aes(y = -log10(pval), x = exp(beta), col = group, size=size, alpha=alpha)) +
...
volcano_plot_solo = function(speci,abbrev,ids,title){
	...
	print(ggplot(localdf, aes(y = -log10(pval), x = exp(beta), col = group, size=size)) +
		ggtitle(title) +
		geom_vline(xintercept=0, linewidth = 0.5,col="gray60",linetype=2)+
		geom_hline(yintercept=1.305, linewidth = 0.5, linetype=2)+
		geom_point() +
