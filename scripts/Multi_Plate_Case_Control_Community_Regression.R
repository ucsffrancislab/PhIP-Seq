...
output_base = paste0(owd, "/", gsub(" ","_",
	paste("Multiplate_Community_Comparison",
...
for(i in c(1:length(plates))){
	#results=read_zfile_communities(paste(plates[i], opt$zfile_basename, sep="/"))
...
for(i in c(1:length(plates))){
	print(paste("Looping",i,":",length(plates)))
